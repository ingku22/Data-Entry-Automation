## chromedriver only works for 1 version of chrome, so when chrome updates, chromedriver must be updated as well
...
chrome_driver = Service(executable_path = "C:\Program Files\Google\Chrome\Application\chromedriver.exe")
...
optionGrpDF = pd.read_excel(excelPath, sheet_name=2)
...
for index in optionGrpDF.index:
    ...
    linkedOptionsDF = pd.read_excel(excelPath, sheet_name=3)
    linkedOptionsDF = linkedOptionsDF[linkedOptionsDF["GroupID"].notna()]

    optionsDF = pd.read_excel(excelPath, sheet_name=4)
    ...
    linkedDishDF = pd.read_excel(excelPath, sheet_name=1)
